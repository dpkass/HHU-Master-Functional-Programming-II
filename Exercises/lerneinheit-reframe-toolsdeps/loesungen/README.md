...
* [`src/cljs/reframe/loesungen/`](src/cljs/reframe/loesungen/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/reframe/loesungen/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `loesungen` project root directory
...
The `debug?` variable in [`config.cljs`](src/reframe/loesungen/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns reframe.loesungen.example
  (:require [reframe.loesungen.config :as config])
